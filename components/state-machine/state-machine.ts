...
export class StateMachine{
    ...
    public setState(name:string, ...args:unknown[]):void{
        ...
        if(!this.#states.has(name)){
            console.warn(`State "${name}" does not exist in state machine "${StateMachine.name}-${this.#id}".`);
            return;
        }
        ...
        this.#log(methodName, `changeing state from ${this.#currentState?.name??'none'} to ${name}`);
    ...
    #log(methodName:string, message:string):void{
        ...
        console.warn(`StateMachine ${StateMachine.name}-${this.#id} - ${methodName}: ${message}`);
    }
}